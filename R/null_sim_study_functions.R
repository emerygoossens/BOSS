...
sim_null_par = function(n = 1000, p = 10, sigma= 1,iters = 10, boss_iters = 10000, unused_cores = 1){
  ...
  simulations = foreach(icount(iters), .combine = rbind) %dopar% {
    ...
    combo_pval_all = pchisq(-2*sum(log(univariate_pvals)), 2*length(univariate_pvals), lower.tail = F)
    output[1] = combo_pval_all
    ...
    options(scipen = 99)
    fsum = summary(lm(y~0 +x))
    fstat = fsum$fstatistic
    f_pval = 1 -pf(fstat[1],fstat[2],fstat[3])
    output[2]= f_pval
    output[3] = p_val_ord_stat_exp_sim[1]
    output[4] = min(p_val_ord_stat_exp_sim*effective_num_tests,.9999)#ifelse(min(p_val_ord_stat_exp_sim)<.1,ent_corrected,output[3])##correct_pv(p_val_ord_stat_exp_sim, fwer = 0.05)
    
    num_selected = which(min(p_val_ord_stat_exp_sim) == p_val_ord_stat_exp_sim)
